...
/* ══════════════════════════════════════════════════════════════════════════
   FIX-26 — ESM/CJS loader bridge for the precious fix packs (THE big one).
   mias/index.js runs as an ES module (mias/package.json "type":"module"), so
   the bare require() calls in its PRECIOUS install block throw
   "require is not defined" — silently swallowed by try/catch. Result: ONLY
   the inline index.js fixes run (play video options + gst); v20/v21/v24 and
   gst-picker never install. This patch inserts a createRequire bridge and
   rewires the four loader calls. Idempotent.
   ══════════════════════════════════════════════════════════════════════════ */
try {
  const _f26 = path.join(__dirname, 'mias', 'index.js');
  if (!fs.existsSync(_f26)) { console.log('[fix_all] FIX-26: mias/index.js not found — skip.'); }
  else {
    let _s26 = fs.readFileSync(_f26, 'utf8');
    if (_s26.includes('__pRequire') || _s26.includes('__createRequireP')) {
      console.log('[fix_all] FIX-26: loader bridge already present ✓');
    } else if (!_s26.includes('PRECIOUS v20 BRIDGE + INSTALLER')) {
      console.log('[fix_all] FIX-26: anchor not found — skip (file layout changed).');
    } else {
      const _bridge =
        '/* ── FIX-26: ESM-safe CJS loader for precious fix packs ───────────── */\n' +
      ...
      const _i = _s26.indexOf('PRECIOUS v20 BRIDGE + INSTALLER');
      const _j = _s26.lastIndexOf('\n/*', _i);
      _s26 = _s26.slice(0, _j + 1) + _bridge + _s26.slice(_j + 1);
      const _sw = [
        ["const _p20 = require('./precious-fixes-v20.cjs');",   "const _p20 = __pRequire('./precious-fixes-v20.cjs');"],
        ["const _p21 = require('./precious-fixes-v21.cjs');",   "const _p21 = __pRequire('./precious-fixes-v21.cjs');"],
        ["const _gstP = require('./precious-gst-picker.cjs');", "const _gstP = __pRequire('./precious-gst-picker.cjs');"],
        ["const _p24 = require('./precious-fixes-v24.cjs');",   "const _p24 = __pRequire('./precious-fixes-v24.cjs');"],
      ];
      for (const [a, b] of _sw) if (_s26.includes(a)) _s26 = _s26.replace(a, b);
      fs.writeFileSync(_f26, _s26, 'utf8');
      console.log('[fix_all] FIX-26: ESM loader bridge installed into mias/index.js ✓');
    }
  }
} catch (_e26) { console.error('[fix_all] FIX-26 error (continuing):', _e26 && _e26.message); }
