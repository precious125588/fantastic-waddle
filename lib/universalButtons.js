...
export async function sendInteractiveCard(sock, jid, body, opts = {}) {
  ...
  // ── Legacy buttonsMessage fallback ──────────────────────────────
  try {
    const sent = await sock.sendMessage(jid, {
      buttonsMessage: {
        ...
        headerType: 1,
        buttons: (opts.buttons || []).slice(0, 3).map((b, i) => ({
          buttonId: b.id || `b${i + 1}`,
          buttonText: { displayText: (b.text || "Tap").slice(0, 25) },
          type: 1,
        })),
      },
    }, sendOpts);
    if (sent?.key) return { ok: true, mode: "legacyButtons", key: sent.key };
  } catch {}
