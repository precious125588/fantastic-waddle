...
// We do NOT import axios here — the host bot already has its own copy
// on disk. The tests stub these globally; production code uses the
// same `axios` symbol exposed by the host module.
//
// In CJS mode (when loaded by mias/index.js) the host global axios is
// reachable via `globalThis.axios`. In ESM mode it must be passed in.
const axios = (typeof globalThis !== "undefined" && globalThis.axios) ||
              (await import("axios").then(m => m.default).catch(() => null)) ||
              null;
...
export async function shazamIdentify(audioBuffer, audioType = "audio/ogg", log = console.log) {
  ...
  const chain = [
    ...
    { name: "nexoracle",run: () => axios.post("https://api.nexoracle.com/misc/shazam?apikey=free_key@maher_apis", { audio: buf.toString("base64") }, { headers: { "Content-Type": "application/json" }, timeout: 30000 }) },
  ];
