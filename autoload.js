...
async function processUser(user, index, total) {
  ...
  try {
    ...
    if (fsSync.existsSync(credsPath)) {
      ...
      await launcher.launch(user, sessionDir);
      console.log(chalk.green(`✅ MAIS MDX launched for: ${user}`));
      return user;
    }

    // No creds → run pairing flow (which itself will spawn MAIS on success)
    ...
    console.log(chalk.green(`✅ Paired & launched: ${user}`));
    return user;
  } catch (error) {
